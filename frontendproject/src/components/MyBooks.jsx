...
try {
    const userBooks = await axios.get(`https://librarymanagementsystem-backend-71te.onrender.com/borrowBooks/getUserBooks/${user?._id}`)
 ...
 try {
  let user= JSON.parse(localStorage.getItem('user'));
  const response = await axios.get(`https://librarymanagementsystem-backend-71te.onrender.com/borrowBooks/returnBook/${user._id}/${bookId}`,{
      headers: {
